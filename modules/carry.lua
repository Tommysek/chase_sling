...
local function formatPlayerInventory(inventory)
    for _, itemData in pairs(inventory) do
        ...
        local itemConfig = CarryConfig[name]
...
local function carryLoop(itemConfig)
    ...
    while carryingItem do
        ...
        if dict and not overideanim and not IsEntityPlayingAnim(ped, dict, anim, 3) then
        ...
        if slowerWalk then
            SetPedMoveRateOverride(PlayerPedId(), slowerWalk)
        end
...
AddStateBagChangeHandler('carry_loop', nil, function(bagName, _, value)
...
AddStateBagChangeHandler('carry_override_anim', nil, function(bagName, _, value)
    if bagName ~= ('player:%s'):format(GetPlayerServerId(PlayerId())) then return end
    overideanim = value
end)

local function updateState(inventory)
    while not playerState or playerState.carry_items == nil do
        Wait(0)
    end
